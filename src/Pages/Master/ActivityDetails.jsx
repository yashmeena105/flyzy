...
const ActivityDetails = ({
  ...
  const isMine = activityData?.company_id?._id == profile.company_id._id;

  const refresh = async () => {
    setLoading(true);
    let resp = await MasterService.getActivity(id);
    if (resp.success) {
      console.log("Activity Data", resp.data);
  ...
  const updatePackages = async (packageData) => {
    setLoading(true);
    let resp = await MasterService.updateMasterActivity({
      payload: { _id: activityData._id, packages: packageData },
    });
    if (resp.success) {
      refresh();
    } else {
  ...
  const dest = JSON.parse(activityData?.destination?.value ?? "{}");
  const loc = {
    ...
    <div style={{ width: "100%", overflowY: "auto" }}>
      ...
      <Space direction="vertical" style={{ width: "100%", padding: "20px" }}>
        <Row gutter={[6, 6]} align="middle" wrap={false} justify="start">
          ...
          <Col>
            <ComponentTypeTag componentType={activityData.type} />
          </Col>
          {!readonly && (
            <Col>
              <Button
              ...
              <Steps
                ...
                current={
                  activityData?.is_published
                    ? 4
                    : activityData?.packages?.length > 0
                    ? 1
                    : 0
                }
                items={[
        ...
        <Row gutter={[12, 6]}>
          ...
          <Col span={18} style={{}}>
            ...
            <ImageCarousal images={activityData?.images ?? []} />
          </Col>
        </Row>
        <Row gutter={[48, 6]}>
          <Col span={6}>
          ...
          <Col span={6}>
            ...
            >
              {(activityData.highlights ?? "").length
