...
const TransportDetails = ({
  ...
  const isMine = transportData?.company_id?._id == profile.company_id._id;

  const refresh = async () => {
    setLoading(true);
    let resp = await MasterService.getTransport(id);
    if (resp.success) {
      console.log("Transport Data", resp.data);
  ...
  const updatePackages = async (packageData) => {
    setLoading(true);
    let resp = await MasterService.updateMasterTransport({
      payload: { _id: transportData._id, packages: packageData },
    });
    if (resp.success) {
      refresh();
    } else {
  ...
  const dest = JSON.parse(transportData?.destination?.value ?? "{}");
  const loc = {
    ...
    <div style={{ width: "100%", overflowY: "auto" }}>
      {publishModalOpen && (
        <PublishModal
          ...
          is_published={transportData?.is_published ?? false}
        />
      )}
      <TransportDrawerForm
        isEditing
      ...
      <Space direction="vertical" style={{ width: "100%", padding: "20px" }}>
        <Row gutter={[6, 6]} align="middle" wrap={false} justify="start">
          ...
          <Col>
            <ComponentTypeTag componentType={transportData.type} />
          </Col>
          {!readonly && (
            <Col>
              <Button
          ...
          <Row gutter={[12, 6]} align="middle">
            ...
            >
              <Steps
                ...
                current={
                  transportData?.is_published
                    ? 4
                    : transportData?.packages?.length > 0
                    ? 1
                    : 0
                }
                items={[
                  ...
                  },
                  {
                    title: (
                      <Button
                        type={
                          transportData?.is_published ?? false
                            ? "default"
                            : "primary"
                        }
                        style={{}}
        ...
        <Row gutter={[12, 6]}>
          ...
          <Col span={18} style={{}}>
            ...
            <ImageCarousal images={transportData?.images ?? []} />
          </Col>
        </Row>
        <Row gutter={[48, 6]}>
          <Col span={6}>
